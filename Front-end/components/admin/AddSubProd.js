...
const AddSubProduct = ({ prodId, edit }) => {
  ...
  const { data: categoriesData, error: categorieError } = useSWR(
    `/categorie/`,
    () =>
      axios.get(`${process.env.NEXT_PUBLIC_BACKEND_URL}/categorie/`, fetcher)
  );

  const { data: subCategoriesData, error: subCategorieMuatate } = useSWR(
      ...
      >
        ...
        <div className="form-item">
          <Select
            ...
            ref={categorieRef}
            data={categoriesData?.data.map((el) => el.nom_categorie)}
            onChange={(e) =>
              setCategorie(
                categoriesData?.data.filter((el) => el.nom_categorie === e)
              )
            }
          />
        </div>
        <div className="form-item">
          <Select
            ...
            data={subCategoriesData?.data.map((el) => el.nom_sous_categorie)}
            onChange={(e) =>
              setSubCategorie(
